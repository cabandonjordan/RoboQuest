...
import { auth, db, doc, getDoc, setDoc } from './database/firebase'; // Import Firebase functions
...
// Joshlee code wink - Part unlocking mapping
const PART_MAPPING = {
  // Innovare category unlocks Innovare parts
  'Innovare': {
    'Weapon': 'WeaponInnovare',
    'Chassis': 'ChassisInnovare',
    'Engines': 'EngineInnovare',
    'Wheels': 'WheelsInnovare',
  },
  // Generalis category unlocks Generalis parts
  'Generalis': {
  ...
  // Creativia category unlocks Creativia parts
  'Creativia': {
    'Weapon': 'WeaponCreativia',
    'Chassis': 'ChassisCreativia',
    'Engines': 'EngineCreativia',
    'Wheels': 'WheelsCreativia',
  },
};

// Joshlee code wink - Function to unlock parts based on detected object
const unlockPartForUser = async (userId, category) => {
  try {
    const userName = userId.displayName || userId.email;
    
    // Get current unlocked parts
    const docRef = doc(db, 'Roboquest-UnlockedParts', userName);
    const docSnap = await getDoc(docRef);
    
    let currentUnlockedParts = {
      Weapon: ['WeaponGeneralis'], // Default unlocked
      Chassis: ['ChassisGeneralis'],
      Wheels: ['WheelsGeneralis'],
      Engines: ['EngineGeneralis'],
    };
    ...
    if (docSnap.exists()) {
      ...
      if (data.unlockedParts) {
        currentUnlockedParts = data.unlockedParts;
      }
    }
    
    // Joshlee code wink - Determine which part to unlock based on category
    let unlockedPart = null;
    let partType = null;
    
    if (PART_MAPPING[category]) {
      // For simplicity, unlock Weapon first, then Chassis, etc.
      if (!currentUnlockedParts.Weapon.includes(PART_MAPPING[category].Weapon)) {
        unlockedPart = PART_MAPPING[category].Weapon;
        partType = 'Weapon';
      } else if (!currentUnlockedParts.Chassis.includes(PART_MAPPING[category].Chassis)) {
        unlockedPart = PART_MAPPING[category].Chassis;
        partType = 'Chassis';
      } else if (!currentUnlockedParts.Engines.includes(PART_MAPPING[category].Engines)) {
        unlockedPart = PART_MAPPING[category].Engines;
        partType = 'Engines';
      } else if (!currentUnlockedParts.Wheels.includes(PART_MAPPING[category].Wheels)) {
        unlockedPart = PART_MAPPING[category].Wheels;
        partType = 'Wheels';
      } else {
        // All parts of this category are already unlocked
        return { alreadyUnlocked: true, category };
      }
      
      if (unlockedPart && partType) {
        // Update unlocked parts
        const updatedUnlockedParts = {
          ...currentUnlockedParts,
          [partType]: [...currentUnlockedParts[partType], unlockedPart]
        };
        
        // Save to Firebase
        await setDoc(docRef, { unlockedParts: updatedUnlockedParts }, { merge: true });
    ...
    return { success: false, message: 'No parts to unlock for this category' };
  } catch (error) {
...
function ResultScreen() {
  ...
  const [unlockResult, setUnlockResult] = useState(null); // Joshlee code wink - Store unlock result
  ...
  // Joshlee code wink - Handle unlocking parts after detection
  const handlePartUnlocking = async (category) => {
    try {
      // Get current user
      ...
      const result = await unlockPartForUser(user, category);
      setUnlockResult(result);
      return result;
    } catch (error) {
      console.error('Error in handlePartUnlocking:', error);
      return null;
    }
  };
    ...
    const performDetection = async () => {
      ...
      try {
        ...
        if (cameraMode === 'single') {
          ...
          if (result.object) {
            // Joshlee code wink - Check category and unlock part if applicable
            if (result.object.category && ['Innovare', 'Generalis', 'Creativia'].includes(result.object.category)) {
              const unlockResult = await handlePartUnlocking(result.object.category);
            ...
            try {
              const journalEntry = {
                ...
                unlockedPart: unlockResult?.unlockedPart || null, // Joshlee code wink - Add unlock info
              };
        ...
        if (result.error) {
          ...
          // Joshlee code wink - Check if any object has unlockable category
          const unlockableObject = result.objects.find(obj => 
            obj.category && ['Innovare', 'Generalis', 'Creativia'].includes(obj.category)
          );
          
          if (unlockableObject) {
            const unlockResult = await handlePartUnlocking(unlockableObject.category);
            console.log('Batch mode unlock result:', unlockResult);
          }
  ...
  // Joshlee code wink - Function to handle navigation back to Loadout with unlock message
  const handleReturnToLoadout = () => {
    if (unlockResult?.success) {
      Alert.alert(
        "🎉 Part Unlocked!",
        `You've unlocked: ${unlockResult.unlockedPart}\n\nReturn to Loadout to equip it!`,
        [
          { 
            text: 'Back to Loadout', 
            onPress: () => {
              navigation.navigate('Loadout', { 
                newlyUnlockedPart: unlockResult.unlockedPart 
              });
            }
          },
          {
    ...
    } else if (unlockResult?.alreadyUnlocked) {
      Alert.alert(
        "All Parts Unlocked",
        `You've already unlocked all ${unlockResult.category} parts!`,
        [{ text: 'OK' }]
      );
    } else {
      navigation.goBack();
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={[styles.content, { justifyContent: 'center' }]}>
          ...
          {/* Joshlee code wink - Add option to return to Loadout */}
          <TouchableOpacity 
            style={[styles.retakeButton, { backgroundColor: '#4B7BEC', marginTop: 10 }]} 
            onPress={() => navigation.navigate('Loadout')}
          >
            <Ionicons name="hammer-outline" size={24} color={colors.background} />
            <Text style={styles.retakeButtonText}>Back to Loadout</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.content}>
          ...
          >
            ...
            {/* Joshlee code wink - Updated back button with Loadout option */}
            <View style={styles.buttonContainer}>
              ...
              <TouchableOpacity 
                style={[styles.backButton, { backgroundColor: '#4B7BEC', marginTop: 10 }]} 
                onPress={handleReturnToLoadout}
              >
                <Ionicons name="hammer-outline" size={24} color={colors.background} />
                <Text style={styles.backButtonText}>Back to Loadout</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
        </View>
...
const styles = StyleSheet.create({
  ...
  // Joshlee code wink - New styles for unlock notification
  unlockNotification: {
  ...
  unlockNotificationHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
    marginBottom: 8,
  },
  unlockNotificationTitle: {
    fontFamily: fonts.heading,
    color: '#4B7BEC',
    fontSize: 18,
    fontWeight: 'bold',
